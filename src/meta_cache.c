...
cache_t * init_cache(cachetype_t cachetype, int set, int way, int offset)
{
	...
	for (long long int i=0; i<set*way; i++)
	{
		...
		my_tag[i].priority = 100000000;  //my_cache->num_way+2; // something out of range   		
...
{
	for (int i = set*my_cache->num_way; i<(set*my_cache->num_way+my_cache->num_way); i++)
	{
...
void promotion_policy(cache_t * my_cache, unsigned long long int addr)
{
	...
	update_arrv_time(my_cache,this_set); // all blocks except for the one which is promoted 

	if ((my_cache->cache_type == VICTIM_CH) && (!VC_MIRROR_RP)) {		// VictimCache - using LRU, not mirroring ReplacementPolicy of Metacache
		// LRU
		int j = 0;
		for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
		{
			if (this_tag == my_cache->tag_array[i].address && my_cache->tag_array[i].valid == 1)
			{
	...
	else if (LRU_REP == 0)
	{
		...
		for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
		{
			...
			pri_arr[j].pri = ALPHA * my_cache->tag_array[i].arv_time + BETA * class_hight + applied_THETA;
			pri_arr[j].way_index = i;
			j++;
		}
	...
	else if (LRU_REP == 3) //SRRIP + level 
	{
		...
		for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
		{
			...
			if (my_cache->tag_array[i].num_hits > thres2)
				applied_THETA = THETA * THRES_THETA_2;
			pri_arr[j].pri = ALPHA * my_cache->tag_array[i].RRPV + BETA * class_hight + applied_THETA;
			pri_arr[j].way_index = i;
			j++;
		}
	...
	for (int i=0; i<my_cache->num_way; i++) // i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
	{
		if (my_cache->tag_array[pri_arr[i].way_index].valid == 1)
			my_cache->tag_array[pri_arr[i].way_index].priority = i; 
		else
		{
			my_cache->tag_array[pri_arr[i].way_index].priority = 100000000;  // very likely to be chosen to evict  	//TODO: Increase if num_cores increased, as shared VC could have 1024 ways
		}
			
		// priority is between 0 to way-1, however, pri_arr is something related to the clock the block arrived  
	}
}
...
void insertion_policy(cache_t * my_cache, unsigned long long int addr)
{
	...
	else if (LRU_REP == 1)
	{
		...
		for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
		{
			...
			pri_arr[j].pri = my_cache->tag_array[i].arv_time;
			pri_arr[j].way_index = i;
		//	printf ("[%d %d %llx]",pri_arr[j].pri,pri_arr[j].way_index, my_cache->tag_array[pri_arr[j].way_index].address);
			j++;
		}
		//printf ("\n");
	}
	...
	else if (LRU_REP == 2) //RRIP
	{
		...
		while (find_way_1 == 0)
		{
			...
			if (find_way_1 == 0)
			{
				for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
				{
					assert ((my_cache->tag_array[i].RRPV != RRIP_N - 1) && (my_cache->tag_array[i].valid == 1));
					my_cache->tag_array[i].RRPV++;
	...
	else if (LRU_REP == 3) //RRIP+level 
	{
		...
		for (int i=0; i<my_cache->num_way; i++) // i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
		{
			...
			if (my_cache->tag_array[pri_arr[i].way_index].valid == 1)
				...
				my_cache->tag_array[pri_arr[i].way_index].priority = i; // priority is between 0 to way-1, however, pri_arr is something related to the clock the block arrived  
			else
			{
...
int victim_selection(cache_t * my_cache, unsigned long long int addr)
{
	...
	for (int i = this_set*my_cache->num_way; i<(this_set*my_cache->num_way+my_cache->num_way); i++)
	{
		if (my_cache->tag_array[index].priority <=  my_cache->tag_array[i].priority)
		{
			index  = i;
		}
	}
...

{
	...
	if (success)
	{
		...
		return(&my_cache->tag_array[found]); 
		// if it is a fine-granularity cache, it still returns the cache line and we need to check if 
		// the 64-bit chunk of data exists or not.  
	}
													...
													int thread_id, int instruction_id, int dirty, microoptype_t typ)
{
	...
	{
		/* code */
			//int this_set = find_set(my_cache->num_offset, my_cache->num_set, addr);//, my_cache->cache_type);
			int index;
