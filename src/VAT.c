...
// Bonsai Merkel tree  
// Finding the region of different zones, finding the hashes which are needed to authenticate date 
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
...
// calculate the number of counter node (not cache line ) in Short ToC (SToC) which is 
// second level of ToC up to root 
// data_cl is the number of cache line which trace is touching 
unsigned long long int num_node_SSGX(unsigned long long int capacity)
{
	unsigned long long int data_cl = ceiling(capacity,64); 
	unsigned long long int SGXs_leaves_mul = (ipow(8, ceiling_long_logyx((data_cl/(8)),8)));
...
// define the region for different zones like data, MAC, hash valuse, and counters 
// capacity is in Byte and it is the size of data the input trace is accessing and according to that
// this function is about to define the size of counter, and MAC for this amount of data 
// the addresses are in BYTE (byte wise) to change them to address of cache line, it is shifted by 6
...
unsigned long long int find_MAC (unsigned long long int addr, unsigned long long int data_start, unsigned long long int capacity )
{
