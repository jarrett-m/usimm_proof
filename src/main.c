  	...
  	if (!vi_file) {
 	  printf("Missing DRAM chip parameter file.  Quitting. \n");
  	  return -5;
  		...
  		vi_file = fopen("/uusoc/scratch/res/arch/students/meysam/input/usimminput/1Gb_x1.vi", "r");
		chips_per_rank= 16;
...
//int byteOffsetWidth = log_base2(CACHE_LINE_SIZE);
  ...
  /// there is a memory which is touched by trace (TRACE_CAPACITY) and usually it is 4G. this is the amount of data trace is touching
  /// but since there are MAC, counter, and hashes the total memory size which is being addressed by memory controller is twice bigger
  /// and it would be 8G for 4G data. This data is for all cores. Unlike the traditional USIMM, the total memory is not multiplied of 
  /// size of memory (say 4G) to the number of cores. Instead, all cores are accessing the original size of memory (say 4G instead of 
  /// 4 * 8 G). As a result, NUM_ROW is not NUM_ROW * NUM_CORE any more.  
...
//num_line_trace++; }
  ...
  for(numc=0; numc<NUMCORES; numc++)
  {
	      if (fgets(newstr,MAXTRACELINESIZE,tif[numc])) 
	      {
					if (sscanf(newstr,"%d %c",&nonmemops[numc],&opertype[numc]) > 0) 
					{
						if (opertype[numc] == 'R') 
						{
							//if (sscanf(newstr,"%d %c %Lx %d",&nonmemops[numc],&opertype[numc],&addr[numc],&dirt[numc]) != 4) 
							if (sscanf(newstr,"%d %c %Lx",&nonmemops[numc],&opertype[numc],&addr[numc]) != 3) 
							{
						...
						addr[numc] = (addr[numc] & (((long long int)0x1<< (ADDRESS_BITS-1))-1)); 
						//addr[numc] = (addr[numc] & (((long long int)0x1<< (ADDRESS_BITS-1-log_base2(NUMCORES)))-1))+
						//(long long int)((long long int)prefixtable[numc] << (ADDRESS_BITS-1-log_base2(NUMCORES))); 
  ...
  while (!expt_done) 
  {
    ...
    for (numc = 0; numc < NUMCORES; numc++) 
    {
			  if ((!ROB[numc].tracedone) && (num_instructions[numc] < (NUM_INST)))
							  ...
							  if (opertype[numc] == 'R') 
							  {
								   ...
								   ROB[numc].length++;
									...
									ROB[numc].instrpc[ROB[numc].tail] = instrpc[numc];
										...
										int lat = read_matches_write_or_read_macro_queue(addr[numc]); //read_matches_write_or_read_queue(addr[numc]);
										if(lat)
										{
												ROB[numc].comptime[ROB[numc].tail] = CYCLE_VAL+(long long int)lat+PIPELINEDEPTH; //????
										}
										else 
										{
										// IV
											...
											insert_macro_read(addr[numc], CYCLE_VAL, numc, ROB[numc].tail, instrpc[numc]);
										}
							    }
							  else
							  { 
								if (opertype[numc] == 'W') 
								{
									...
									ROB[numc].comptime[ROB[numc].tail] = CYCLE_VAL+PIPELINEDEPTH;
									  ...
									  // sensitive 
									    if(!write_exists_in_write_macro_queue(addr[numc])) //write_exists_in_write_queue(addr[numc])) //IV
										{
												//insert_write(addr[numc], CYCLE_VAL, numc, ROB[numc].tail);
												//IV
												insert_macro_write(addr[numc], CYCLE_VAL, numc, ROB[numc].tail);
										} 
    ...
    if (num_done == NUMCORES) {
      ...
      if ((numch == NUM_CHANNELS) && (size_macro_rd_queue() == 0) && (size_macro_wr_queue() == 0) && (mt_tab == NULL) )
      {
 ...
 for (int h=0; h<12; h++){
  ...
  printf ("UMERGED read with a read  %lld read with a write %lld  write with a write %lld \n", 
	     num_macro_read_merge_read, num_macro_read_merge_write, num_macro_write_merge);
