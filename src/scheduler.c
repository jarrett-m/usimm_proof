...
void schedule(int channel)
{
	...
	if(drain_writes[channel])
	{
		/*if (CYCLE_VAL > 2540000000)
		{
			printf ("___________________ write queue in scheduler ____________\n");
		...
		LL_FOREACH(write_queue_head[channel], wr_ptr)
		{
			if(wr_ptr->command_issuable)
			{
						...
						if (ptr->dram_addr.channel == wr_ptr->dram_addr. channel &&
							ptr->dram_addr.rank == wr_ptr->dram_addr.rank &&
							 ptr->dram_addr.bank == wr_ptr->dram_addr.bank &&
							  ptr->dram_addr.row == dram_state[wr_ptr->dram_addr. channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank].active_row)
							  {
								  keep_open = 1;
								  break;
							  }
				...
				{
					/* if (wr_ptr->next_command == ACT_CMD && num_ACT_CMD[channel] >= LOW_NUM)
						{
						...
						if (ptr->dram_addr.channel == rd_ptr->dram_addr. channel &&
							ptr->dram_addr.rank == rd_ptr->dram_addr.rank &&
							 ptr->dram_addr.bank == rd_ptr->dram_addr.bank &&
							  ptr->dram_addr.row == dram_state[rd_ptr->dram_addr. channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank].active_row)
							  {
								  keep_open = 1;
								  break;
							  }
