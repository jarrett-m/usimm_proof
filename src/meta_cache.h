...
typedef struct tag {
  ...
  int priority; // shows the priority in the pririty stack for replacement
                // policy
  int dirty;
  // 8 bit the least segnificat digits of dirty and valid shows dirty and
  // valid bits of 8 64-bit chyncks od MAC address if FG_PARITY_CACHE is
  // activated.
  int valid;
  int thread_id;
  int instruction_id;
...
#define find_set(numoffset, numset, addr)                                      \
  ((addr >> log_base2(numoffset)) &                                            \
   (((long long int)0x1 << log_base2(numset)) - 1))
